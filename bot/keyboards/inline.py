...
def ch_search_results_kb(channels: list, page: int = 0,
    ...
    for ch in page_channels:
        # Иконки: комментарии + подписчики + метка "уже добавлен"
        comments_icon = "💬" if ch.get("has_comments") else "🔇"
        members = ch.get("members_formatted", "?")
        already = "✅ " if ch.get("already_added") else "➕ "
        title = ch.get("title", "")
        if len(title) > 20:
            title = title[:20] + "…"

        text = f"{already}{comments_icon} @{ch['username']} [{members}] {title}"
        callback = f"ch_search_add_{ch['username']}"
    ...
    if total_pages > 1:
        ...
        if page > 0:
            nav.append(InlineKeyboardButton(
                text="◀️ Назад", callback_data=f"ch_search_page_{page - 1}"))
        nav.append(InlineKeyboardButton(
            text=f"{page + 1}/{total_pages}", callback_data="noop"))
        if page < total_pages - 1:
            nav.append(InlineKeyboardButton(
                text="Вперёд ▶️", callback_data=f"ch_search_page_{page + 1}"))
        buttons.append(nav)
...
def camp_logs_kb(camp_id: int, page: int, total_pages: int) -> InlineKeyboardMarkup:
    buttons = []
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(
            text="◀️", callback_data=f"camp_logs_{camp_id}_p{page - 1}"))
    nav.append(InlineKeyboardButton(
        text=f"{page + 1}/{total_pages}", callback_data="noop"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton(
...
def camp_mode_kb(camp_id: int, current_mode: str) -> InlineKeyboardMarkup:
    """Мультивыбор режимов для кампании (чекбоксы)."""
    selected_modes = set(current_mode.split(",")) if current_mode else {"comments"}
    buttons = []
    for mode, label in MODE_LABELS.items():
        check = "✅" if mode in selected_modes else "⬜"
        buttons.append([InlineKeyboardButton(
            text=f"{check} {label}",
...
def prs_mode_kb(prs_id: int, current_mode: str) -> InlineKeyboardMarkup:
    """Мультивыбор режимов для пресета (чекбоксы)."""
    selected_modes = set(current_mode.split(",")) if current_mode else {"comments"}
    buttons = []
    for mode, label in MODE_LABELS.items():
        check = "✅" if mode in selected_modes else "⬜"
...
def proxy_pool_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🚫 Снять прокси со всех", callback_data="prx_clear_all")],

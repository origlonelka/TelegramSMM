...
async def check_account(acc, timeout: int = 30) -> dict:
    ...
    async def _do_check():
        ...
        try:
            ...
            # get_me() может вернуть данные даже для замороженных аккаунтов
            # (is_deleted не выставляется для собственного профиля).
            # Поэтому делаем реальный запрос к account API — для
            # деактивированных аккаунтов это вызовет USER_DEACTIVATED.
            ...
            if getattr(me, "is_deleted", False):
                return {
                    "ok": False,
                    "dead": True,
                    "error": "Аккаунт заморожен / удалён (is_deleted)",
                }
            if getattr(me, "is_restricted", False):
                reason = ""
                if getattr(me, "restriction_reason", None):
                    reason = "; ".join(
                        r.text for r in me.restriction_reason if hasattr(r, "text")
                    )
                return {
                    "ok": False,
                    "dead": True,
                    "error": f"Аккаунт ограничен (is_restricted){': ' + reason if reason else ''}",
                }

            return {"ok": True, "phone": phone}
        except asyncio.CancelledError:
            # Таймаут — тихо отключаемся
            try:
                await client.disconnect()
            except Exception:
                pass
    ...
    if done:
        ...
        # Таймаут — отменяем задачу и ждём завершения
        task.cancel()
        try:
            await task
...
async def ensure_connected(acc, timeout: int = 30) -> Client:
    client = get_client(acc)
    if not client.is_connected:
        acc_id = acc["id"]
        try:
            # Используем connect() вместо start() — start() при невалидной
            # сессии вызывает input(), который блокирует event loop навечно.
            # connect() только устанавливает TCP-соединение.
            ...
            raise ConnectionError(
                f"Таймаут подключения ({timeout} сек) — проверьте прокси"
            )
        except Exception:
            _clients.pop(acc_id, None)
            raise

import os
import logging
from pyrogram.errors import AuthKeyUnregistered, UserDeactivated, UserDeactivatedBan, SessionRevoked
from db.database import execute, fetch_one, fetch_all, delete_account
...
async def apply_template(acc, template) -> dict:
    ...
    try:
        ...
        logger.error(f"Аккаунт #{acc['id']} мёртв ({type(e).__name__}), помечаю")
        await disconnect(acc["id"])
        await execute(
            "UPDATE accounts SET status = 'unauthorized' WHERE id = ?",
            (acc["id"],))
        return {"ok": False, "error": str(e), "dead": True}
